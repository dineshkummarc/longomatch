...
//  report_progress (stream, 0);
  ...
  if ((proxy_env = getenv("http_proxy")) != NULL)
    this->proxy_url = g_strdup(proxy_env);
  else
    this->proxy_url = NULL;
  this->s               = -1;
  this->asf_header_len  = 0;
...
//  report_progress (stream, 0);
  ...
  if (this->proxy_url) {
    char * uri_string;
    this->proto = (uri->scheme) ? g_strdup(uri->scheme) : NULL;
    this->connect_host = (proxy_uri->hostname) ? g_strdup(proxy_uri->hostname) : NULL;
    this->connect_port = proxy_uri->port;
    this->http_host = (uri->scheme) ? g_strdup(uri->hostname) : NULL;
    this->http_port = uri->port;
    this->proxy_user = (proxy_uri->user) ? g_strdup(proxy_uri->user) : NULL;
    this->proxy_password = (proxy_uri->passwd) ? g_strdup(proxy_uri->passwd) : NULL;
    this->host_user = (uri->user) ? g_strdup(uri->user) : NULL;
    this->host_password = (uri->passwd) ? g_strdup(uri->passwd) : NULL;
    gnet_uri_set_scheme(uri,"http");
    uri_string = gnet_uri_get_string(uri);
    this->uri = g_strdup(uri_string);
    g_free(uri_string);
  } else {
    this->proto = (uri->scheme) ? g_strdup(uri->scheme) : NULL;
    this->connect_host = (uri->hostname) ? g_strdup(uri->hostname) : NULL;
    this->connect_port = uri->port;
    this->http_host = (uri->hostname) ? g_strdup(uri->hostname) : NULL;
    this->http_port = uri->port;
    this->proxy_user = NULL;
    this->proxy_password = NULL;
    this->host_user =(uri->user) ?  g_strdup(uri->user) : NULL;
