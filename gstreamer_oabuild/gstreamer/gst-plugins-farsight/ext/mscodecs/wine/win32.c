...
static long WINAPI expWideCharToMultiByte(long v1, long v2, unsigned short* s1, long siz1,
...
static long WINAPI expRegOpenKeyExW(long key, const char* subkey, long reserved, long access, int* newkey)
{
    dbgprintf("RegOpenKeyExW(key 0x%x, subkey %p, reserved %d, access 0x%x, pnewkey 0x%x) => -1\n",
 ...
 * of the wineserver.
 */
static unsigned int WINAPI expGetTickCount(void)
{
  ...
  tc = 1000 + (((t.tv_sec - start_time.tv_sec) * 1000) + ((t.tv_usec - start_time.tv_usec) / 1000));
...
static void * exp_decode_pointer(void * ptr)
{
  return ptr;
}

static void exp_lock (int index)
{
  dbgprintf("_lock(%d)\n", index);
}

static void exp_unlock (int index)
{
  dbgprintf("_unlock(%d)\n", index);
}
...
static void* exp_malloc_crt(int size)
{
  return expmalloc(size);
}
...
static void* exp__dllonexit(int x, int y, int z)
{
    dbgprintf("__dllonexit(%d, %d, %d) => 0\n", x, y, z);
...
static double exp_CIsin(void)
{
    ...
    double result = sin(x);
    dbgprintf("_CIsin(%lf) => %lf\n", x, result);
    return result;
}

static double exp_CIlog(void)
{
    FPU_DOUBLE(x);

    double result = log(x);
    dbgprintf("_CIlog(%lf) => %lf\n", x, result);
    return result;
}

static double exp_CIlog10(void)
{
    FPU_DOUBLE(x);

    double result = log10(x);
    dbgprintf("_CIlog10(%lf) => %lf\n", x, result);
    return result;
}
...
static double exp_CIsqrt(void)
{
    ...
    dbgprintf("_CIsqrt(%lf) => %lf\n", x, result);
    return result;
}

static double exp_CIexp(void)
{
    FPU_DOUBLE(x);

    double result = exp(x);
    dbgprintf("_CIexp(%lf) => %lf\n", x, result);
    return result;
}
...
struct exports exp_MSVCR80[]={
    ...
    FF(free, -1)
    FF(_initterm, -1)
    { "_initterm_e", -1, (void*)&exp_initterm },
    FF(memset, -1)
    FF(memcpy, -1)
    FF(memmove, -1)
    FF(_CIpow,-1)
