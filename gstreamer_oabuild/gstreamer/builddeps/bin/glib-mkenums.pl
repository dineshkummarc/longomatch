...
sub version {
    print "glib-mkenums version glib-2.12\n";
...
sub usage {
    ...
    print "  --flist-env-var <NAME>     file list in environment variable NAME\n";
...
while ($_=$ARGV[0],/^-/) {
    ...
    elsif (/^--flist-env-var$/)      { $flist_env_var = shift }
    elsif (/^--help$/ || /^-h$/)     { usage; }
...
if (length($flist_env_var)) {
    if (!exists($ENV{$flist_env_var})) {
	die "Environment variable $flist_env_var not defined";
    }
    @ARGV = split(/;/, $ENV{$flist_env_var});
}
