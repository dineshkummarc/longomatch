...
void
gst_message_print (GstMessage * msg, GstElement * play, const char *filename)
{
  GError *err = NULL;
  char *dbg = NULL;
...
GdkPixbuf *
gst_playbin_get_frame (GstElement * play)
{
  GstStructure *s;
  GstBuffer *buf = NULL;
  GdkPixbuf *pixbuf;
...
static GdkPixbuf *
gst_buffer_to_pixbuf (GstBuffer * buffer)
{
...
static const GValue *
gst_tag_list_get_cover_real (GstTagList * tag_list)
{
  ...
  for (i = 0;; i++) {
    ...
    value = gst_tag_list_get_value_index (tag_list, GST_TAG_IMAGE, i);
    if (value == NULL)
      break;
...
GdkPixbuf *
gst_tag_list_get_cover (GstTagList * tag_list)
{
  const GValue *cover_value;

  g_return_val_if_fail (tag_list != NULL, FALSE);

  cover_value = gst_tag_list_get_cover_real (tag_list);
  /* Fallback to preview */
  if (!cover_value) {
  ...
  if (cover_value) {
    ...
    pixbuf = gst_buffer_to_pixbuf (buffer);
    return pixbuf;
  }
