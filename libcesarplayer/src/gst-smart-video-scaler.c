...
gst_smart_video_scaler_init (GstSmartVideoScaler *object)
{
	GstSmartVideoScalerPrivate *priv;
  	object->priv = priv = G_TYPE_INSTANCE_GET_PRIVATE (object, GST_TYPE_SMART_VIDEO_SCALER, GstSmartVideoScalerPrivate);
...
gsvs_compute_and_set_values (GstSmartVideoScaler *gsvs)
{
	...
	fdarin = (gfloat)gst_value_get_fraction_numerator(gsvs->priv->darin)/(gfloat)gst_value_get_fraction_denominator (gsvs->priv->darin);
	fdarout = (gfloat)gst_value_get_fraction_numerator(gsvs->priv->darout)/(gfloat)gst_value_get_fraction_denominator (gsvs->priv->darout);
	fparin = (gfloat)gst_value_get_fraction_numerator(gsvs->priv->parin)/(gfloat)gst_value_get_fraction_denominator (gsvs->priv->parin);
	fparout = (gfloat)gst_value_get_fraction_numerator(gsvs->priv->parout)/(gfloat)gst_value_get_fraction_denominator (gsvs->priv->parout);
  	GST_INFO("incoming width/height/PAR/DAR : %d/%d/%f/%f",gsvs->priv->widthin,gsvs->priv->heightin,fparin, fdarin);
   	GST_INFO("outgoing width/height/PAR/DAR : %d/%d/%f/%f",gsvs->priv->widthout,gsvs->priv->heightout,fparout,fdarout);
    ...
    if (fdarin == fdarout && (mayor >=0 && minor >= 10 && micro >= 23)){
      	GST_INFO("We have same input and output caps, resetting capsfilter and videobox settings");
        ...
        if (fdarin > fdarout){
	        gint newheight;
           	...
           	GST_INFO("incoming DAR is greater that ougoing DAR. Adding top/bottom borders");           	
        	...
        	left = right = 0;
         	...
         	GST_INFO("incoming DAR is smaller than outgoing DAR. Adding left/right borders");
  	...
  	/* set properties on elements */
   	GST_INFO("About to set left/right/top/bottom : %d/%d/%d/%d", -left, -right, -top, -bottom);
...
static void
gsvs_get_value_from_caps(GstCaps *caps,gboolean force, gint *width, gint *height, GValue **par, GValue **dar)
{
    ...
    *height = -1;
    ...
    if (force || (caps && gst_caps_is_fixed(caps))){
		...
		if (g_strrstr (gst_structure_get_name (str), "pixel-aspect-ratio")){
	 		...
	 		gst_value_set_fraction( *par, gst_value_get_fraction_numerator(tmp_par),gst_value_get_fraction_denominator (tmp_par));	 			 		
 		}
...
gst_smart_video_scaler_new ()
{
	...
	/*Create bin elements*/
	gsvs->priv->videoscale = gst_element_factory_make("videoscale", "smart-videoscale");
   	g_object_set (G_OBJECT(gsvs->priv->videoscale), "method",1,NULL);      	
 	gsvs->priv->capsfilter = gst_element_factory_make("capsfilter", "smart-capsfilter");
 	gsvs->priv->videobox = gst_element_factory_make("videobox", "smart-videobox");
 	
 	/*Add and link elements*/
 	gst_bin_add_many(GST_BIN(gsvs),gsvs->priv->videoscale,gsvs->priv->capsfilter, gsvs->priv->videobox, NULL);
  	gst_element_link_many(gsvs->priv->videoscale, gsvs->priv->capsfilter, gsvs->priv->videobox,NULL);

	/*Create bin sink pad*/
	sink_pad = gst_element_get_static_pad(gsvs->priv->videoscale,"sink");
	gst_pad_set_active (sink_pad, TRUE);
    gst_element_add_pad (GST_ELEMENT(gsvs), gst_ghost_pad_new ("sink", sink_pad));	
