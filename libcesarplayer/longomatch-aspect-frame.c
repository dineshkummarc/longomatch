...
#define ASPECT_FRAME_PRIVATE(o)                         \
  (G_TYPE_INSTANCE_GET_PRIVATE ((o),                    \
     ...
     struct _LongomatchAspectFramePrivate
     {
       guint expand:1;
     ...
     static void
         longomatch_aspect_frame_get_property (GObject * object,
    guint property_id, GValue * value, GParamSpec * pspec)
{
  LongomatchAspectFrame *frame = LONGOMATCH_ASPECT_FRAME (object);

  switch (property_id) {
    case PROP_EXPAND:
      g_value_set_boolean (value, longomatch_aspect_frame_get_expand (frame));
      break;

    case PROP_RATIO:
      g_value_set_float (value, longomatch_aspect_frame_get_ratio (frame));
      break;
...
static void
longomatch_aspect_frame_set_property (GObject * object,
    guint property_id, const GValue * value, GParamSpec * pspec)
{
  switch (property_id) {
    case PROP_EXPAND:
      longomatch_aspect_frame_set_expand (LONGOMATCH_ASPECT_FRAME (object),
          g_value_get_boolean (value));
      break;

    case PROP_RATIO:
      longomatch_aspect_frame_set_ratio (LONGOMATCH_ASPECT_FRAME (object),
...
static void
longomatch_aspect_frame_dispose (GObject * object)
{
  G_OBJECT_CLASS (longomatch_aspect_frame_parent_class)->dispose (object);
}

static void
longomatch_aspect_frame_finalize (GObject * object)
{
  G_OBJECT_CLASS (longomatch_aspect_frame_parent_class)->finalize (object);
}

static void
longomatch_aspect_frame_get_preferred_width (ClutterActor * actor,
    gfloat for_height, gfloat * min_width_p, gfloat * nat_width_p)
{
  ...
  CLUTTER_ACTOR_CLASS
      (longomatch_aspect_frame_parent_class)->get_preferred_width (actor,
      for_height, min_width_p, nat_width_p);
...
longomatch_aspect_frame_get_preferred_height (ClutterActor * actor,
    gfloat for_width, gfloat * min_height_p, gfloat * nat_height_p)
{
  gboolean override;
  MxPadding padding;
  ...
  CLUTTER_ACTOR_CLASS
      (longomatch_aspect_frame_parent_class)->get_preferred_height (actor,
      for_width, min_height_p, nat_height_p);
}
    ...
    const ClutterActorBox * box, ClutterAllocationFlags flags)
{
  ...
  CLUTTER_ACTOR_CLASS (longomatch_aspect_frame_parent_class)->allocate (actor,
      box, flags);
...
longomatch_aspect_frame_paint (ClutterActor * actor)
{
  ...
  if (priv->expand) {
    ...
    if (CLUTTER_IS_TEXTURE (child)) {
      ...
      material = clutter_texture_get_cogl_material (CLUTTER_TEXTURE (child));
      ...
      cogl_rectangle_with_texture_coords (padding.left, padding.top,
          padding.left + width,
          padding.top + height, 0.5f - tx, 0.5f - ty, 0.5f + tx, 0.5f + ty);
    } else {
...
static void
longomatch_aspect_frame_pick (ClutterActor * actor, const ClutterColor * color)
{
  ClutterActorBox box;
  ...
  if (priv->expand) {
    ...
    cogl_clip_push_rectangle (padding.left, padding.top,
        padding.left + (box.x2 - box.x1), padding.top + (box.y2 - box.y1));
    clutter_actor_paint (child);
    cogl_clip_pop ();
  } else
    clutter_actor_paint (child);
...
static void
longomatch_aspect_frame_class_init (LongomatchAspectFrameClass * klass)
{
  GParamSpec *pspec;
  ...
  actor_class->get_preferred_width =
      longomatch_aspect_frame_get_preferred_width;
  actor_class->get_preferred_height =
      longomatch_aspect_frame_get_preferred_height;
  ...
  pspec = g_param_spec_float ("ratio",
      ...
      "(width/height).",
      -1.f, G_MAXFLOAT, -1.f, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);
...
void
longomatch_aspect_frame_set_expand (LongomatchAspectFrame * frame,
    gboolean expand)
{
