...
namespace CesarPlayer
{
    ...
    public class DSPlayer : UserControl, IPlayer
    {
		...
		public float Position{
			...
			set { 
        		this.Seek((long)(value*this.clipLength),false);
        	}
		...
		public void CancelProgramedStop(){
			int hr = 0;
            hr = this.mediaSeeking.SetPositions(null, AMSeekingSeekingFlags.NoPositioning, this.Length*MS, AMSeekingSeekingFlags.AbsolutePositioning);
		    DsError.ThrowExceptionForHR(hr);
		}
        ...
        public void Seek(long pos,bool accurate)
        {
		...
		public void SegmentSeek (long start, long stop){
			int hr = 0;
            this.Pause();
            hr = this.mediaSeeking.SetPositions(start*MS, AMSeekingSeekingFlags.AbsolutePositioning, stop*MS, AMSeekingSeekingFlags.AbsolutePositioning);
		    DsError.ThrowExceptionForHR(hr);
		    this.Play();
		}
		...
		public double Rate {
			...
			get{return this.currentPlaybackRate;
			}
		}
        ...
        */
		...
		protected virtual void OnTimerElapsed(object sender, ElapsedEventArgs e) {
        	this.updateTime();
			this.TickEvent(this.currentTime, this.clipLength, this.currentPosition,true);		
		}
